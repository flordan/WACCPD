...
public class MainActivity extends Activity {
    ...
    public void start(View aview) {
        ...
        EditText maxSourcesET = (EditText) findViewById(R.id.maxSources_text);
        EditText varET = (EditText) findViewById(R.id.variation_text);
        EditText iterationsET = (EditText) findViewById(R.id.iterations_text);
        new Compute().execute(maxSourcesET.getText().toString(), varET.getText().toString(),
                iterationsET.getText().toString());
    }

    private class Compute extends AsyncTask<String, Void, String> {

        Long time;

        protected String doInBackground(String... params) {
            time = System.currentTimeMillis();
            int maxSources = Integer.parseInt(params[0]);
            float variation = Float.parseFloat(params[1]);
            int iterations = Integer.parseInt(params[2]);
            return Computation.start(maxSources, variation, iterations);
        }
