...
public class OpenCL {
    ...
    public static cl_mem addInput(byte[] data) {
        int[] errCode = new int[1];
        cl_mem buffer = CL.clCreateBuffer(context, CL_MEM_READ_ONLY, Sizeof.cl_char * data.length, null, errCode);
        CL.clEnqueueWriteBuffer(queue, buffer, CL_TRUE, 0, Sizeof.cl_char * data.length, Pointer.to(data), 0, null, null);
        return buffer;
    }
    ...
    public static cl_mem addOutput(byte[] data) {
        int[] errCode = new int[1];
        return CL.clCreateBuffer(context, CL_MEM_READ_ONLY, Sizeof.cl_char * data.length, null, errCode);
    }
    ...
    public static void retrieveData(cl_mem outDevice, byte[] out) {
        CL.clEnqueueReadBuffer(queue, outDevice, CL_TRUE, 0, Sizeof.cl_char * out.length, Pointer.to(out), 0, null, null);
    }
